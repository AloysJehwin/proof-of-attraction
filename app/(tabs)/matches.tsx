    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <Pressable style={styles.row} onPress={() => router.push(`/chat/${item.matchId}`)}>
          {item.other.photos[0] ? (
            <Image source={{ uri: item.other.photos[0] }} style={styles.avatar} />
          ) : (
          ...
          <View style={styles.rowBody}>
            <View style={styles.rowTop}>
              ...
              <TierBadge tier={item.other.tier} small />
            </View>
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', gap: spacing.md, padding: spacing.md, backgroundColor: colors.surface, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, alignItems: 'center' },
  avatar: { width: 56, height: 56, borderRadius: radius.pill },
  noPhoto: { backgroundColor: colors.surfaceAlt },
